...
# Filter for older adults and create RVA/IVA classification (exact match to original)
datcomb_oa <- datcomb %>%
  filter(year >= 2007 & year <= 2022) %>%
  filter(AGE >= 65) %>%  # No as.numeric conversion
...
# First, we need to prepare the variables for regression using the same approach as your original
# Load stringr for string functions
library(stringr)

# Create survey design with properly coded variables (matching your original methodology)
survey_design_reg <- survey_design %>%
  # Convert to factors and create numeric codes as in original
  update(
    VisitType_factor = as.factor(VisitType),
    SEX_factor = as.factor(SEX),
    RACER_factor = as.factor(RACER),
    # Create age groups and triage acuity as FACTOR (modified from original)
    TriAc_factor = factor(
...
survey_design_reg2 <- survey_design %>%
  # Exclude year 2020 using subset (proper method for survey objects)
  subset(year != "2020") %>%
  # Convert to factors and create numeric codes as in original
  update(
    VisitType_factor = as.factor(VisitType),
